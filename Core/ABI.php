...
class ABI
{
        ...
        if ($round == 1) {
            ...
            if (Utils::string_contains($input_type, '[]')) {
                ...
                for ($i = 0; $i < (int)$matches[1][0]; $i++) {
                    ...
                    $inferredInput->type = $typeStr;
                    $inferredType[$i] = $inferredInput;
                }
                $res = self::EncodeGroup($inferredType, $inputData);
                if (self::IsStaticParameter(self::GetParameterType($typeStr)))
                    return $res;
                $input->hash = $res;
    ...
    private static function EncodeInput_Int($data)
    {
        if (is_string($data) && ctype_digit($data)) {
            ...
            throw new Exception("EncodeInput_Int, ($data) not valid input type");
        }
    ...
    {
        $partial = substr($encoded, (int)$start, 64);
    ...
    private static function DecodeInput_UInt($encoded, $start)
    {
        ...
        $partial = self::RemoveZeros($partial, true);
        $partial_big = new BigNumber((string)$partial, 16);
        return $partial_big;
    }
    ...
    private static function DecodeInput_String($encoded, $start)
    {
        ...
        $partial = substr($encoded, (int)$start, (int)($length * 2));
    ...
    {
        $partial = self::RemoveZeros(substr($encoded, (int)$start, 64), false);
        return hex2bin($partial);
    }
